...
def load_protonbeam_annotations(annotations_path="fullscale-data/protonbeam.csv", use_grouped_data=False):
    annotations = None
    if use_grouped_data:
        annotations = pd.read_csv("fullscale-data/protonbeam_grouped.csv", delimiter="\t", header=None)
    else:
...
def load_texts_and_pmids(citations_and_labels_path="fullscale-data/protonbeam_data.csv"):
    protonbeam = pd.read_csv(citations_and_labels_path, delimiter=",")
    texts = []
            ...
            if pmid in train_pmids:
                ...
                for worker, question_answers in q_decisions_for_pmid.groupby("workerId"):
                    ...
                    if model == "cf-independent-responses":
                        ...
                        q4 = question_answers[['q4']].values[0][0]
                        q1a = -1 if (q1 == "No" or q1 == "\\N") else 1
                        q4a = -1 if (q2 == "No" or q2 == "\\N") else 1
                        q3a = -1 if (q3 == "No" or q3 == "\\N") else 1
                        q2a = -1 if (q4 == '\\N' or (q4 != 'NoInfo' and q4 < 10)) else 1
                        train_y.append(q1a)
                        train_y.append(q2a)
                        train_y.append(q3a)
                        train_y.append(q4a)
...
def get_grouped_rationales_model(annotations, X, train_y, pmids, train_pmids, train_indices, vectorizer, use_worker_qualities=True, use_grouped_data=False, n_jobs=1):
    ...
    for question_num in range(1,5):
        if use_grouped_data and question_num != 1:
            pass
        elif question_num == 4: # This is Proton beam specific.
            # @TODO something else?
            pass
        else:
